...
class SimRecord:
    ...
    def __init__(self, directory):
        ...
        self.table = Table(names=['idx', 'observationId', 'sourceId', 'blendId', 'neighborId', 'fieldx', 'fieldy', 'posx', 'posy', 'parallactic', 'airmass', 'zenith', 'seed', 'chip', 'intensity', 'temperature'],
                           dtype=['i4', 'i8', 'i8', 'i4', 'i4', 'f4', 'f4', 'f4', 'f4', 'f4', 'f4', 'f4', 'i4', 'str', 'i4', 'f4'])

    def write(self, observationId, sourceId, blendId, neighborId, fieldx, fieldy, posx, posy, parallactic, airmass, zenith, seed, chip, intensity, temperature, starImage, zernikes):
        ...
        self.table.add_row([self.idx, observationId, sourceId, blendId, neighborId, fieldx, fieldy, posx, posy, parallactic, airmass, zenith, seed, chip, intensity, temperature])
        self.idx += 1
...
if __name__ == '__main__':
    ...
    parser.add_argument('-blend', type=int, default=10) # number of blends to use
    ...
    # check arguments
    if args.blend <= 0 or args.atm <= 0 or not os.path.exists(args.dir):
    ...
    sr = SimRecord(os.path.join(args.dir, f'records/blend{args.obs}'))

    # find blends
    blends = Table(names=['index', 'neighbors'], dtype=['i4','i4'])
    for i,row in enumerate(catalog):
        dist = np.sqrt((catalog['ra'] - row['ra']) ** 2 + 
                       (catalog['dec'] - row['dec']) ** 2)
        # count neighbors within 128 pixel radius
        blends.add_row([i,np.sum(dist < 0.0071)])

    blends.sort('neighbors', reverse=True)
    blend_count = np.sum(blends['neighbors'] > 1)
    samples = [blends[0]['index']] # always include star with most neighbors
    # then randomly choose args.blend more
    samples.extend(np.random.choice(blends[1:blend_count]['index'], 
                             min(args.blend, blend_count - 1), replace=False))

    for i,sample in enumerate(samples):
        # check if we should regenerate the atmosphere
        if i % ceil(args.blend / args.atm) == 0:
        ...
        center = catalog[sample]
        catalog['dist'] = np.sqrt((catalog['ra'] - center['ra']) ** 2 + 
                       (catalog['dec'] - center['dec']) ** 2)
        neighbors = catalog[catalog['dist'] < 0.0071]
        neighbors.sort('dist') # central object is first
        ...
        for j,row in enumerate(neighbors):
            ...
            # write
            sr.write(observation['observationId'], row['source_id'], i, j, field.x, field.y, 
        ...
        finish_blend = time()
        print(f'Blend length: {len(neighbors)}, time: {finish_blend - start_blend}')
